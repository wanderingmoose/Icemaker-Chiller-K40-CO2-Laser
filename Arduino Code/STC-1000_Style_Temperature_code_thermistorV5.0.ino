...
/*
  ...
  V 5.0 (Might change placement on LCD for readings)
  ...
  ***Outputs***
  ...
  LCD is 1602 using a ic2 interface on A5 and A4
  VGA Display support using OctaPentaVeega board. Pin 8 serial output
  **Inputs***
  Four switch pad so not using the left button. But still works great for me. (designed for a 5 button switches)
  Pulse flow meter
  2xThermistor 10k. First Temp is for comp control, the second is for visual info.
  ***Libraries***
  EEPROMex for eeprom https://playground.arduino.cc/Code/EEPROMex/
  LCD used the LiquidCrystalic_IC2.https://github.com/johnrickman/LiquidCrystal_I2C
...
/*--------------------------------------------------------------------------------------
  Defines
  --------------------------------------------------------------------------------------*/
#define tempPin1                 A2  //A2 is the input for Thermistor 10k Water reading of water coming into cooling system
#define tempPin2                 A1  // Just a temperature reading for water leaving tank
//#define flowPin1               A3  // Turbine input with a 10uf cap on the input to smooth out the voltage.
...
const byte AlarmUP[6] = {137, 133, 133, 133, 133, 151};
const byte AlarmMD[6] = {138, 32, 32, 32, 32, 138};
const byte AlarmX[6] = {144, 133, 133, 133, 133, 140};
const byte AlarmLW[6] = {136, 133, 133, 133, 133, 135};
...
THERMISTOR thermistor2(tempPin2,        // Analog pin
                       ...
                       10000);          // Value of the series resistor
/*--------------------------------------------------------------------------------------
  Init the LCD library with the LCD Ic2
  --------------------------------------------------------------------------------------*/
...
void setup()
{
  ...
  lcd.print( "IceMaker Control" );
  lcd.setCursor( 0, 1 );   //bottom left
  lcd.print( "K40 Chiller V4.4" );
  delay(5000);
  lcd.clear();
  lcd.setCursor(0, 0);
...
void loop()
{
                  ...
                  case MENU_BUZZER:
                    {
                      ...
                      {
                        lcd.setCursor(0, 0);
                        lcd.print("Buzzer1=on0=off");
                        lcd.setCursor(0, 1);
                        lcd.print(set_buzzer);
                         SetCursor (14,3);
                  ...
                  case MENU_ALARMS:
                    {
                      ...
                      {
                        lcd.setCursor(0, 0);
                        lcd.print("Alarms1=on0=off)");
                        lcd.setCursor(0, 1);
                        lcd.print(set_alarms);
                         SetCursor (14,3);
                  ...
                  case MENU_EXIT:
                    {
                      ...
                      EEPROM.writeFloat(12, set_high_alarm);                //address 12
                      EEPROM.writeFloat(16, set_low_alarm);                 //address 16
                      EEPROM.writeInt(20, set_min_flow);                    //address 20
                      EEPROM.writeInt(24, set_door);                        //address 24
                      EEPROM.writeInt(28, set_buzzer);                      //address 28
                      EEPROM.writeInt(32, set_alarms);                      //address 32
                      lcd.clear();
                      delay(200);
                      break;
                    }
 ...
 //
  ...
  lcd. print( "     " );
  lcd.setCursor(11, 0);
  lcd.print (temperatureC);                    //Print averaged temperature on LCD
  //VGA Display Temp
  SetColor (37,40);
  SetCursor(5,2);
  vga.print("    ");
  ...
  lcd. print( "      " );
  lcd.setCursor(11, 1);
  lcd.print (temperatureC1);                    //Print averaged temperature on LCD with 1 decimal
   //VGA Display Temp
  SetColor (37,40);
  SetCursor(5,13);
  vga.print("    ");
...
{ lcd.setCursor(0, 1);
  ...
  SetColor (37, 40);
    SetCursor (1,2);
    vga.print (" IceMaker Controller V5.0 ");
  digitalWrite(alarm_PIN, LOW);  
  alaRms=0;
  digitalWrite(buzzer_PIN,LOW);
}
...
/* 
 //small box
  SetCursor (6, 25);
  SetColor (37, 40);
  vga.write(AlarmUP, 6); // Create a box on the UI
  for (int f = 7; f < 8; f++)
  {
    SetCursor (f, 25);
    vga.write(AlarmMD, 6);
  ...
  */
  //TEXT
  SetCursor (1, 4);  // some text and stuff
  SetColor (37, 40);
  vga.print ("IceMaker Controller V5.0");
   SetCursor (3,1);
   vga.print ("I/P Temp");
